...
export function sha256(value) { const bytes = typeof value === "string" ? value : canonicalJson(value); return createHash("sha256").update(bytes).digest("hex"); }
export function requireString(value, path, max = 512) {
  if (typeof value !== "string" || value.length === 0 || value.length > max) throw new ContractError("INVALID_FIELD", `${path} must be a non-empty string <= ${max} chars`, { path });
  return value;
}
export function validateArtifactRef(ref) {
  ...
  requireString(ref.media_type, "artifact.media_type", 256); requireString(ref.digest, "artifact.digest", 128);
  ...
  requireString(ref.uri, "artifact.uri", 4096); return structuredClone(ref);
}
...
export function validateTransition(from, to) { if (!TRANSITIONS.get(from)?.has(to)) throw new ContractError("ILLEGAL_TRANSITION", `illegal transition ${from} -> ${to}`, { from, to }); }
export function validateContextDelta(snapshot, delta) {
  ...
  validateArtifactRef(delta.delta_ref); return structuredClone(delta);
}
export function makeEffectKey({ task_id, logical_attempt_id, turn_no, operation, subject_digest }) { return `effect:v1:${sha256({ task_id, logical_attempt_id, turn_no, operation, subject_digest })}`; }
