...
const requiredImports = new Map([
  ["KPK-009", ["workflows/temporal/src/worker-deployment.js", "docs/ADR-0005-TEMPORAL-WORKER-DEPLOYMENTS.md"]],
  ["KPK-010", ["packages/drive-adapter/src/artifacts.js", "docs/ADR-0006-DRIVE-IMMUTABLE-ADAPTER.md"]],
  ["KPK-022", ["packages/eval-flywheel/evals/akashic-core.jsonl", "docs/ADR-0007-EVAL-REGRESSION-FLYWHEEL.md"]]
]);
for (const [packetId, paths] of requiredImports) {
  const packet = registry.packets.find((item) => item.packet_id === packetId);
  if (packet?.adoption_status !== "IMPORT_THIS_BRANCH") throw new Error(`${packetId} must be marked IMPORT_THIS_BRANCH`);
...
console.log(JSON.stringify({ ok: true, packets: ids.length, skills: skills.length, imported_packets: [...requiredImports.keys()], bakeoff_weight: totalWeight, source_sha256: registry.source.sha256 }));
